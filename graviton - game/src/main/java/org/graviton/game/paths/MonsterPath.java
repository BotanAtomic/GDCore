...
public class MonsterPath {
    ...
    public MonsterPath(AbstractMap map, short startCell, short lastCell) {
        this.map = map;
    ...
    public List<Cell> getShortestPath() {
        ...
        while (!openList.isEmpty() && !closeList.containsKey(lastCell)) {
         ...
         /**   if (currentNode.getCellId() == lastCell && Cells.cellAroundIsOccupied(map, currentNode.getCellId())) {
            ...
            for (int loc0 = 0; loc0 < 4; loc0++) {
                short cellId = Cells.getCellIdByOrientation(currentNode.getCellId(), OrientationEnum.ADJACENT[loc0], map.getWidth()); //TODO ADJACENT :D
        ...
        System.err.println("Close list size = " + closeList.size());
        return simplePath();
    }
    ...
    public static Pair<String, Byte> compilePath(List<Cell> path, short firstCell, AbstractMap map) {
        ...
        for (Cell currentCell : path) {
            OrientationEnum orientation = Cells.getOrientationByCells(currentCellId, currentCell.getId(), map);
            if (orientation != null) {
